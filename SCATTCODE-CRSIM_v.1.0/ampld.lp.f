C---------------------------------------------------------------------
C---------------------------------------------------------------------
C This file  is a part of double precisssion T-matrix code for non-
C sperical particles in a fixed orientation developed by Michael
C Mishchenko at the NASA Goddard Institute for Space Studies, 
C New York (https://www.giss.nasa.gov/staff/mmishchenko/ftpcode/)
C 
C The purpose of the original file ampld.lp.f was to demonstrate
C the use of T-matrix code for a specific set of the input parameters.
C 
C MODIFICATIONS
C
C 
C The original main program is transformed to a subroutine 
C tm_ampld(AXI,RAT,LAM,MRR,MRI,EPS,NP,DDELT,NDGS,NMAX) which computes 
C the T-matrix for the given scattering particle. 
C Computation of the elements of the amplitude and phase matrix for 
C desired different directions of the incident and scattered beams is 
C moved from the original main program to  to the new main program 
C fort_tmatrix.f90. 
C No other changes exept those above mentioned and related to easier 
C interface with Mishchenko's T-matrix code are performed. 
C
C Oct 19, 2019 Aleksandra Tatarevic 
C------------------------------------------------------------------------
C------------------------------------------------------------------------
C
      subroutine tm_ampld(AXI,RAT,LAM,MRR,MRI,EPS,NP,DDELT,NDGS,NMAX)
...
c      WRITE(*,*) 'NMAX=',NMAX 
c      WRITE(*,*) '-------------------------------' 
C      CALL AMPL (NMAX,LAM,THET0,THET,PHI0,PHI,ALPHA,BETA,
C     &           S11,S12,S21,S22)     
...
C   CALCULATION OF THE AMPLITUDE MATRIX       
     ...
     *       CTP1,CPP1
     ...
     &    PL1.LT.0D0.OR.PL1.GT.360D0) THEN
          WRITE (*,*) 'AN ANGULAR PARAMETER IS OUTSIDE ITS All range'
          STOP
      ELSE
          CONTINUE
      ENDIF
